...
using Revise
println("""
**Development REPL for NoiseTAI** (Revise active)
""")
endef
...
coverage: test/Manifest.toml  ## Run the test suite with coverage
	$(JULIA) --project=test -e 'using LocalCoverage; report = generate_coverage("NoiseTAI"; run_test=true); show(report)'

htmlcoverage: test/Manifest.toml  ## Run the test suite with coverage and generate an HTML report in ./coverage
	$(JULIA) --project=test -e 'using LocalCoverage; html_coverage("NoiseTAI"; dir="coverage")'
